...
if cmd_opts.ngrok is not None:
    ...
    ngrok.connect(
        cmd_opts.ngrok, cmd_opts.port if cmd_opts.port is not None else 7860, cmd_opts.ngrok_options
    )
    ...
    if mode in {0, 1, 3, 4}:
        ...
        for image in images:
            ...
            print(
                ...
                file=open(os.path.join(ii_output_dir, f"{left}.txt"), 'a', encoding='utf-8'),
            )
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Tab("ประมวลผล", id="txt2img_generation") as txt2img_generation_tab, ResizeHandleRow(
            equal_height=False
        ):
                ...
                for category in ordered_ui_categories():
                    if category == "prompt":
                        ...
                        with gr.Row(elem_id="txt2img_accordions", elem_classes="accordions"):
                            with InputAccordion(
                                False, label="ชัด HD", elem_id="txt2img_hr"
                            ) as enable_hr:
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row1", variant="compact"):
                                    ...
                                    denoising_strength = gr.Slider(
                                        ...
                                        maximum=1.0,
                                        step=0.01,
                                        label='ความแรงในการลดสัญญาณรบกวน',
                                        value=0.7,
                                ...
                                with FormRow(
                                    ...
                                    hr_checkpoint_name = gr.Dropdown(
                                        ...
                                        + modules.sd_models.checkpoint_tiles(use_short=True),
            ...
            txt2img_paste_fields = [
                ...
                PasteField(
                    ...
                    and ("Hires upscale" in d or "Hires upscaler" in d or "Hires resize-1" in d),
                    api="enable_hr",
                ),
                ...
                PasteField(
                    hr_sampler_name, sd_samplers.get_hr_sampler_from_infotext, api="hr_sampler_name"
                ),
                PasteField(
                    hr_scheduler, sd_samplers.get_hr_scheduler_from_infotext, api="hr_scheduler"
                ),
                PasteField(
                    ...
                    lambda d: (
                        ...
                        or d.get("Hires checkpoint", "Use same checkpoint") != "Use same checkpoint"
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Tab("ประมวลผล", id="img2img_generation") as img2img_generation_tab, ResizeHandleRow(
            equal_height=False
        ):
            with ExitStack() as stack:
                ...
                for category in ordered_ui_categories():
                    ...
                    if category == "image":
                        with gr.Tabs(elem_id="mode_img2img"):
                            ...
                            with gr.TabItem(
                                'ภาพ > ภาพ', id='img2img', elem_id="img2img_img2img_tab"
                            ) as tab_img2img:
                                init_img = gr.Image(
                                    label="รูปภาพสำหรับ img2img",
                            ...
                            with gr.TabItem(
                                'สเก็ตช์', id='img2img_sketch', elem_id="img2img_img2img_sketch_tab"
                            ) as tab_sketch:
                                sketch = gr.Image(
                                    label="รูปภาพสำหรับ img2img",
                            ...
                            with gr.TabItem(
                                'เติมภาพ', id='inpaint', elem_id="img2img_inpaint_tab"
                            ) as tab_inpaint:
                                init_img_with_mask = gr.Image(
                                    label="รูปภาพสำหรับ inpainting พร้อมมาสก์",
                                    show_label=False,
                                    elem_id="img2maskimg",
                                    source="upload",
                                    interactive=True,
                            ...
                            with gr.TabItem(
                                ...
                                inpaint_color_sketch = gr.Image(
                                    label="การวาดสเก็ตช์สีสำหรับ inpainting",
                                    show_label=False,
                            ...
                            with gr.TabItem(
                                'รอบ', id='batch', elem_id="img2img_batch_tab"
                            ) as tab_batch:
                                with gr.Tabs(elem_id="img2img_batch_source"):
                                    ...
                                    with gr.TabItem(
                                        ...
                                        hidden = (
                                            '<br>ปิดการใช้งานเมื่อเปิดด้วย --hide-ui-dir-config.'
                                ...
                                with gr.Accordion("ข้อมูล PNG", open=False):
                                    img2img_batch_use_png_info = gr.Checkbox(
                                        label="เพิ่มข้อมูล PNG ใน prompt",
                                        elem_id="img2img_batch_use_png_info",
                                    )
                                    img2img_batch_png_info_dir = gr.Textbox(
                                        label="ไดเรกทอรีข้อมูล PNG",
                                        **shared.hide_dirs,
                                        placeholder="ปล่อยว่างเพื่อใช้ไดเรกทอรีอินพุต",
                                    ...
                                    img2img_batch_png_info_props = gr.CheckboxGroup(
                                        ...
                                        label="พารามิเตอร์ที่จะใช้จากข้อมูล PNG",
                                        info="Prompt จากข้อมูล PNG จะถูกเพิ่มเข้าไปใน Prompt ที่ตั้งไว้ใน UI",
                                    )
                        ...
                        with FormRow():
                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                ...
                                with gr.Tabs(elem_id="img2img_tabs_resize"):
                                    with gr.Tab(
                                        label="ปรับขนาดเป็น", id="to", elem_id="img2img_tab_resize_to"
                                    ) as tab_scale_to:
                                        with FormRow():
                                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                                width = gr.Slider(
                                    ...
                                    with gr.Tab(
                                        label="ปรับขนาดคูณ", id="by", elem_id="img2img_tab_resize_by"
                                    ) as tab_scale_by:
                                        scale_by = gr.Slider(
                                            minimum=0.05,
                                            maximum=4.0,
                        ...
                        denoising_strength = gr.Slider(
                            ...
                            maximum=1.0,
                            step=0.01,
                            label='ความแข็งแรงในการลดสัญญาณรบกวน',
                            value=0.75,
                ...
                with gr.Tab(label="Train", id="train"):
                    ...
                    with FormRow():
                        batch_size = gr.Number(
                            label='ภาพ/รอบ', value=1, precision=0, elem_id="train_batch_size"
                        )
    ...
    interfaces = [
        (txt2img_interface, "ข้อความ > ภาพ", "txt2img"),
        (img2img_interface, "ภาพ > ภาพ", "img2img"),
        (extras_interface, "เสริม", "extras"),
    ...
    interfaces += [(settings.interface, "ตั้งค่า", "settings")]
    ...
    with gr.Blocks(theme=custom_theme, analytics_enabled=False, title="SIIN AI") as demo:
        ...
        with gr.Tabs(elem_id="tabs") as tabs:
            ...
            for interface, label, ifid in sorted_interfaces:
                if label in shared.opts.hidden_tabs or label not in [
                    'ข้อความ > ภาพ',
                    'ตั้งค่า',
                    'ภาพ > ภาพ',
                    'เสริม',
...
def setup_ui_api(app):
    ...
    app.add_api_route(
        "/internal/sysinfo-download", lambda: download_sysinfo(attachment=True), methods=["GET"]
    )

...
class ScriptPostprocessingCodeFormer(scripts_postprocessing.ScriptPostprocessing):
    ...
    def ui(self):
        with ui_components.InputAccordion(False, label="CodeFormer") as enable:
            with gr.Row():
                codeformer_visibility = gr.Slider(minimum=0.0, maximum=1.0, step=0.001, label="การมองเห็น", value=1.0, elem_id="extras_codeformer_visibility")
                codeformer_weight = gr.Slider(minimum=0.0, maximum=1.0, step=0.001, label="น้ำหนัก (0 = maximum effect, 1 = minimum effect)", value=0, elem_id="extras_codeformer_weight")
